...
void MergeSG::extract(std::vector<Object>& objects, PointCloudT::Ptr point_ptr,std::vector<Cluster>& clusters)
{   
...
// 1. 点云团分割，获取点云团================================= 
   ...
   PointCloudT::Ptr cloud_segment(new PointCloudT);// 保存的点云，有序点云====
    ...
    std::cout<<"segment Time="<<timeUsed1/1000<<" ms"<<std::endl;
         ...
         indices_it != cluster_indices.end(); ++indices_it )
    {
        try
        {
            ...
            double mean_z =  sumz / seg->size(); //均值
             ...
             //std::cout<<"x:"<<object3d.boxCenter[0]<<"y:"<<object3d.boxCenter[1]<<"z:"<<object3d.boxCenter[2]<<std::endl;
...
// 2. 与2d检测框融合，生成带有标签的3d目标物体
    ...
    std::cout<<"  findMaxInter Time="<<timeUsed2/1000<<" ms"<<std::endl;
}

void MergeSG::extract(std::vector<Object>& objects, 
                      PointCloudT::Ptr point_ptr,    // 有序点云=====
                      std::vector<Cluster>& clusters,std::vector<pcl::PointIndices> cluster_indices)
{   
    ...
    //std::vector<pcl::PointIndices> cluster_indices; // 点云团索引 数组=====
    PointCloudT::Ptr cloud_segment(new PointCloudT);// 保存的点云，有序点云====
    pcl::copyPointCloud(*point_ptr, *cloud_segment);  // 保存的点云，有序点云====
   
    std::vector<Object3d> object3ds;// 3d点云团

    for ( std::vector<pcl::PointIndices>::iterator indices_it = cluster_indices.begin();indices_it != cluster_indices.end(); ++indices_it )
    {
        try
        {
             pcl::PointCloud<PointXYZPixel>::Ptr seg(new pcl::PointCloud<PointXYZPixel>);
                      ...
                      std::vector<pcl::PointIndices>& cluster_indices)// 点云团索引 数组
{
    ...
    gettimeofday(&tend1,NULL);
    timeUsed1=1000000*(tend1.tv_sec-tstart1.tv_sec)+tend1.tv_usec-tstart1.tv_usec;
    std::cout<<"segmentPlanes Time="<<timeUsed1/1000<<" ms"<<std::endl;
    ...
    gettimeofday(&tend2,NULL);
    timeUsed2=1000000*(tend2.tv_sec-tstart2.tv_sec)+tend2.tv_usec-tstart2.tv_usec;
    std::cout<<"segmentObjects Time="<<timeUsed2/1000<<" ms"<<std::endl;
                             ...
                             pcl::PointCloud<pcl::Normal>::Ptr& normal_cloud)
{
    ...
    //设置深度变化的阀值
    normal_estimation_.setMaxDepthChangeFactor(0.05f);
    // 设置计算法线的区域
    normal_estimation_.setNormalSmoothingSize(5.0f);
    ...
    gettimeofday(&tend2,NULL);
    timeUsed2=1000000*(tend2.tv_sec-tstart2.tv_sec)+tend2.tv_usec-tstart2.tv_usec;
    std::cout<<"compute Time="<<timeUsed2/1000<<" ms"<<std::endl;
    ...
    std::vector<pcl::PointIndices>& label_indices) // 索引
{
    ...
    //plane_segmentation_.setMinInliers(1000);
    //plane_segmentation_.setAngularThreshold(0.017453*10);
   // plane_segmentation_.setDistanceThreshold(2);
    ...
    std::cout << "MPS+Refine took: " << double(mps_end - mps_start)*1000 <<"ms"<< std::endl;
}
